...
namespace Gurux.Service.DB
{
    ...
    public class GXQueryCache
    {
        ...
        /// <summary>
        /// Thread-safe dictionary storing cached query strings, keyed by the full cache key.
        /// </summary>
        readonly ConcurrentDictionary<string, GXCacheItem> Items =
            new ConcurrentDictionary<string, GXCacheItem>(StringComparer.Ordinal);
        ...
        public GXQueryCache(TimeSpan cacheTime, DatabaseType databaseType)
        {
            CacheTime = cacheTime;
        ...
        /// </code>
        /// </example>
        internal string BuildKey(params object[] values)
        {
        ...
        private void AppendSignature(StringBuilder sb, object value, HashSet<object> visited, int depth)
        {
            ...
            if (value is LambdaExpression lambda)
            {
                ...
                AppendExpressionValues(sb, lambda.Body, visited, depth + 1);
                return;
            }
            if (value is Expression expression)
            {
                ...
                AppendExpressionValues(sb, expression, visited, depth + 1);
                return;
            }
            ...
            {
                sb.Append("c:");
                sb.Append(((int)character).ToString(CultureInfo.InvariantCulture));
                return;
            }
            if (value is Enum enumValue)
            {
                ...
                sb.Append(':');
                sb.Append(Convert.ToInt64(enumValue, CultureInfo.InvariantCulture).ToString(CultureInfo.InvariantCulture));
                return;
            }
            ...
            AppendObjectSignature(sb, value, vt, visited, depth);
        }

        private void AppendObjectSignature(StringBuilder sb, object value, Type vt, HashSet<object> visited, int depth)
        {
            List<string> members = new List<string>();
            foreach (PropertyInfo it in vt.GetProperties(BindingFlags.Instance | BindingFlags.Public))
            {
                ...
                tmp.Append(it.Name);
                tmp.Append('=');
                AppendSignature(tmp, it.GetValue(value, null), visited, depth + 1);
                members.Add(tmp.ToString());
            }
            foreach (FieldInfo it in vt.GetFields(BindingFlags.Instance | BindingFlags.Public))
            {
                StringBuilder tmp = new StringBuilder();
                tmp.Append(it.Name);
                tmp.Append('=');
                AppendSignature(tmp, it.GetValue(value), visited, depth + 1);
                members.Add(tmp.ToString());
            }
            ...
            for (int pos = 0; pos != members.Count; ++pos)
            {
                ...
                sb.Append(members[pos]);
            }
            sb.Append('}');
        }

        private void AppendExpressionValues(StringBuilder sb, Expression expression, HashSet<object> visited, int depth)
        {
            if (expression == null || depth > 64)
            {
                return;
            }
            switch (expression)
            {
                case BinaryExpression binary:
                    AppendExpressionValues(sb, binary.Left, visited, depth + 1);
                    AppendExpressionValues(sb, binary.Right, visited, depth + 1);
                    break;
                case UnaryExpression unary:
                    AppendExpressionValues(sb, unary.Operand, visited, depth + 1);
                    break;
                case MethodCallExpression call:
                    AppendExpressionValues(sb, call.Object, visited, depth + 1);
                    foreach (Expression it in call.Arguments)
                    {
                        AppendExpressionValues(sb, it, visited, depth + 1);
                    }
                    break;
                case MemberExpression member:
                    AppendExpressionValues(sb, member.Expression, visited, depth + 1);
                    if (member.Expression is ConstantExpression closure)
                    {
                        object captured = null;
                        if (member.Member is FieldInfo field)
                        {
                            captured = field.GetValue(closure.Value);
                        }
                        else if (member.Member is PropertyInfo property)
                        {
                            captured = property.GetValue(closure.Value, null);
                        }
                        sb.Append(":cv:");
                        AppendSignature(sb, captured, visited, depth + 1);
                    }
                    break;
                case NewExpression newExpression:
                    foreach (Expression it in newExpression.Arguments)
                    {
                        AppendExpressionValues(sb, it, visited, depth + 1);
                    }
                    break;
                case NewArrayExpression array:
                    foreach (Expression it in array.Expressions)
                    {
                        AppendExpressionValues(sb, it, visited, depth + 1);
                    }
                    break;
                case MemberInitExpression memberInit:
                    AppendExpressionValues(sb, memberInit.NewExpression, visited, depth + 1);
                    foreach (MemberBinding binding in memberInit.Bindings)
                    {
                        if (binding is MemberAssignment assignment)
                        {
                            AppendExpressionValues(sb, assignment.Expression, visited, depth + 1);
                        }
                    }
                    break;
                case ConstantExpression constant:
                    if (constant.Value != null && constant.Value.GetType().IsValueType)
                    {
                        sb.Append(":const:");
                        AppendSignature(sb, constant.Value, visited, depth + 1);
                    }
                    break;
            }
        }
